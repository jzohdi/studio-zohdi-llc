/**
 * Generates the 1200×630 social share card at `static/og-image.png`.
 *
 * Renders a branded HTML template in headless Chromium (Playwright is already a
 * dev dependency) and screenshots it. Re-run whenever the brand or tagline
 * changes:  `node scripts/generate-og-image.mjs`
 */
import { chromium } from 'playwright';
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';

const __dirname = dirname(fileURLToPath(import.meta.url));
const outputPath = join(__dirname, '..', 'static', 'og-image.png');

const WIDTH = 1200;
const HEIGHT = 630;

const mark = `
<svg viewBox="0 0 500 500" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  <circle cx="240" cy="263" r="229" fill="white"/>
  <path d="M204.068 344.364C196.318 353.754 186.922 361.03 175.88 366.197C164.849 371.364 152.989 373.947 140.297 373.947C124.338 373.947 109.661 369.489 96.2759 360.572C82.8853 351.879 72.1978 339.666 64.2134 323.926L90.63 310.53C95.7967 321.338 102.844 329.676 111.776 335.551C120.453 341.666 129.963 344.718 140.297 344.718C151.807 344.718 162.151 341.546 171.318 335.197C180.234 329.103 187.276 319.822 192.443 307.364C197.609 295.155 200.193 280.364 200.193 262.989V142.489H229.443V262.989C229.443 279.655 227.328 294.921 223.109 308.78C218.635 322.645 212.286 334.504 204.068 344.364Z" fill="black"/>
  <path d="M318.577 172.072H218.514V142.489H367.202L263.264 342.593H362.973V371.843H214.639L318.577 172.072Z" fill="black"/>
  <path d="M362.602 371.843C358.612 371.843 355.211 370.317 352.394 367.26C349.571 364.442 348.165 361.041 348.165 357.051C348.165 353.051 349.691 349.645 352.748 346.822C355.566 344.004 358.852 342.593 362.602 342.593C366.602 342.593 370.128 344.004 373.185 346.822C376.003 349.879 377.415 353.291 377.415 357.051C377.415 361.041 376.003 364.442 373.185 367.26C370.128 370.317 366.602 371.843 362.602 371.843Z" fill="black"/>
</svg>`;

const html = `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
<link rel="preconnect" href="https://fonts.googleapis.com" />
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
<link
  href="https://fonts.googleapis.com/css2?family=League+Spartan:wght@500;600;700;800&display=swap"
  rel="stylesheet"
/>
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  html, body { width: ${WIDTH}px; height: ${HEIGHT}px; }
  body {
    position: relative;
    overflow: hidden;
    font-family: 'League Spartan', sans-serif;
    color: #f5f7fb;
    background:
      radial-gradient(120% 140% at 88% 8%, hsla(318, 78%, 53%, 0.32), transparent 46%),
      radial-gradient(90% 120% at 8% 100%, hsla(217, 80%, 55%, 0.18), transparent 50%),
      linear-gradient(155deg, #0b1018 0%, #05070c 100%);
    display: flex;
    flex-direction: column;
    justify-content: space-between;
    padding: 76px 84px;
  }
  /* subtle hairline frame */
  .frame {
    position: absolute;
    inset: 28px;
    border: 1px solid rgba(255, 255, 255, 0.08);
    border-radius: 22px;
    pointer-events: none;
  }
  .top { display: flex; align-items: center; gap: 26px; }
  .mark { width: 84px; height: 84px; filter: drop-shadow(0 10px 26px rgba(0,0,0,0.45)); }
  .mark svg { width: 100%; height: 100%; }
  .eyebrow {
    font-size: 26px;
    font-weight: 600;
    letter-spacing: 0.42em;
    text-transform: uppercase;
    color: rgba(245, 247, 251, 0.72);
  }
  .headline {
    font-size: 168px;
    font-weight: 800;
    line-height: 0.84;
    letter-spacing: -0.05em;
    text-transform: uppercase;
  }
  .headline .accent { color: hsl(318, 78%, 62%); }
  .tagline {
    margin-top: 30px;
    font-size: 42px;
    font-weight: 600;
    letter-spacing: -0.01em;
    color: rgba(245, 247, 251, 0.9);
  }
  .rule {
    width: 132px;
    height: 6px;
    margin-top: 30px;
    border-radius: 999px;
    background: linear-gradient(90deg, hsl(318, 78%, 56%), hsl(217, 85%, 60%));
  }
  .bottom { display: flex; align-items: flex-end; justify-content: space-between; }
  .projects {
    font-size: 25px;
    font-weight: 500;
    letter-spacing: 0.02em;
    color: rgba(245, 247, 251, 0.62);
    max-width: 760px;
    line-height: 1.4;
  }
  .domain {
    font-size: 27px;
    font-weight: 700;
    letter-spacing: 0.04em;
    color: hsl(318, 78%, 64%);
    white-space: nowrap;
  }
</style>
</head>
<body>
  <div class="frame"></div>
  <div class="top">
    <div class="mark">${mark}</div>
    <div class="eyebrow">Freelance Web Studio</div>
  </div>

  <div>
    <div class="headline">Studio <span class="accent">Zohdi</span></div>
    <div class="tagline">Premium freelance web development</div>
    <div class="rule"></div>
  </div>

  <div class="bottom">
    <div class="projects">
      FOIA Search&nbsp;&nbsp;·&nbsp;&nbsp;Stock Promotion Tracker&nbsp;&nbsp;·&nbsp;&nbsp;High Ground Research&nbsp;&nbsp;·&nbsp;&nbsp;8KSearch
    </div>
    <div class="domain">studio-zohdi-llc.com</div>
  </div>
</body>
</html>`;

const browser = await chromium.launch();
try {
	const page = await browser.newPage({
		viewport: { width: WIDTH, height: HEIGHT },
		deviceScaleFactor: 1
	});
	await page.setContent(html, { waitUntil: 'networkidle' });
	await page.evaluate(() => document.fonts.ready);
	await page.screenshot({ path: outputPath, clip: { x: 0, y: 0, width: WIDTH, height: HEIGHT } });
	console.log(`Wrote ${outputPath} (${WIDTH}x${HEIGHT})`);
} finally {
	await browser.close();
}
